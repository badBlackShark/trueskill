module Gauss
  class TruncatedCorrection
    def self.w_within_margin(perf_diff, draw_margin)
    ...
    def self.v_within_margin(perf_diff, draw_margin)
      abs_diff = perf_diff.abs
      denom = Distribution.cdf(draw_margin - abs_diff) - Distribution.cdf(-draw_margin - abs_diff)
      if denom < 2.2e-162
    ...
    def self.w_exceeds_margin(perf_diff, draw_margin)
